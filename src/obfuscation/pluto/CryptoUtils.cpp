...
void CryptoUtils::prng_seed() {
    unsigned seedVal;
    char* envSeed = nullptr;
#ifdef _WIN32
    size_t len = 0;
    _dupenv_s(&envSeed, &len, "PLUTO_SEED");
#else
    envSeed = std::getenv("PLUTO_SEED");
#endif
    if (envSeed) {
        seedVal = static_cast<unsigned>(std::strtoul(envSeed, nullptr, 0));
        fprintf(stderr, "[Pluto] Using seed from PLUTO_SEED: 0x%08X\n", seedVal);
#ifdef _WIN32
        free(envSeed);
#endif
    } else {
        ...
        seedVal = rd();
        fprintf(stderr, "[Pluto] Generated random seed: 0x%08X\n", seedVal);
    }
    std::mt19937 gens(seedVal);
    this->gen = gens;
    seeded = true;
}
