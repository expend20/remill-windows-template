...
llvm::SwitchInst *BlockTerminator::FinishBlock(
  ...
  switch (last_instr.instr.category) {
    ...
    case remill::Instruction::kCategoryIndirectJump: {
      // External indirect jumps (tail calls) are handled in pre-lift detection
      // This handles non-external indirect jumps
      llvm::Value *target_pc = remill::LoadProgramCounter(block, *intrinsics);
    ...
    case remill::Instruction::kCategoryIndirectFunctionCall: {
      // External indirect calls are handled in pre-lift detection
      // This handles non-external indirect calls
